...
app.post('/api/loanusers/', function (req, res) {
   ...
   // for(var i=0;i<totalmydate; i++) {
		//	var InsertUsername = mydate[i];
		//pool.getConnection(function (err, connection) {
			connection.query("INSERT INTO loandetail SET loan_id = ?, loan_amount = ?, startdate = ?, enddate = ?,  installementtype = ?, noofinstallement = ?,dayDifference = ?, interest = ?, cid = ?;",[loan_id, loan_amount, startdate, enddate, installementtype, noofinstallement, dayDifference, interest, cid], function (err, rows, fields) {
				if (!!err) {
					data["newuser"] = "Error Adding data";
					console.log(err);
					//log.error(err);
...
app.post('/api/loanuserss/', function (req, res) {
   ...
   // for(var i=0;i<totalmydate; i++) {
		//	var InsertUsername = mydate[i];
		//pool.getConnection(function (err, connection) {
			connection.query("INSERT INTO loandetail SET loan_id = ?, loan_amount = ?, startdate = ?, enddate = ?,  installementtype = ?, noofinstallement = ?,dayDifference = ?,  cid = ?;",[loan_id, loan_amount, startdate, enddate, installementtype, noofinstallement, dayDifference, cid], function (err, rows, fields) {
				if (!!err) {
					data["newuser"] = "Error Adding data";
					console.log(err);
					//log.error(err);
...
app.post('/lent', function (req, res) {
   ...
   connection.query('SELECT loan_amount, loandetail.loan_id, registration.cid,  noofinstallement, receivedamt, fullname, loandetail.startdate,  mobile FROM registration JOIN  loandetail ON registration.cid = loandetail.cid  WHERE  loandetail.startdate   BETWEEN ? AND ? ',[fdate,tdate],function(err, rows, fields){
if (rows.length !== 0 && !err) {
				data["error"] = 0;
				data["report"] = rows;
				res.json(data);
...
app.post('/mobile/customerdelete', function (req, res) {
    ...
    var cid = req.body.cid;
	console.log(cid);
  	connection.query("DELETE FROM registration  WHERE cid = ?",[cid], function (error, results, fields) {
...
app.post('/mobile/loandetaildelete', function (req, res) {
	
    var loan_id = req.body.loan_id;
	console.log(loan_id);
  	connection.query("DELETE FROM loandetail  WHERE loan_id = ?",[loan_id], function (error, results, fields) {
...
app.post('/mobile/transactionmodeldelete', function (req, res) {
	...
	console.log(transaction_id);
  	connection.query("DELETE FROM transactionmodel  WHERE transaction_id = ?",[transaction_id], function (error, results, fields) {
